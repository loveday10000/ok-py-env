...
sudo apt-get -y install vim nginx curl node nodejs npm git ssh docker.io phantomjs redis mongodb mysql-server mysql-client libmysqlclient-dev
...
# 6.自动重启系统
gnome-terminal -- bash -c "sh ok-py-env-master/py-web.sh"
